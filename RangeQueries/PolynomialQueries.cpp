...
vl a(mx);
void app(int i,ll left,ll right,ll val,ll lz2){
    seg[i].s+=(right-left+1)*val+lz2*((right+1)*right/2-(left-1)*left/2);
    seg[i].lz+=val;
    seg[i].lz2+=lz2;
}
...
void rangeUpdate(int i,int l,int r,int left,int right,int val,ll lz){
...
void solve(){
    ...
    forl(i,0,q){
        ...
        if(t==1){
            ll a,b;
            cin>>a>>b;
            rangeUpdate(0,a-1,b-1,0,n-1,2-a,1);
        }
        else{
            ll a,b;
            cin>>a>>b;
